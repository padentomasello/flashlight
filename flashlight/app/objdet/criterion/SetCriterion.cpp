...
af::array index(const af::array& in, af::array idx, const int dim) {
...
fl::Variable index(const fl::Variable& in, const af::array idx, const int dim) {
  std::vector<af::array> idxs(4);
  idxs[dim] = idx;
  return index(in, idxs);
}
...
SetCriterion::LossDict SetCriterion::forward(
  ...
  for (int i = 0; i < predBoxesAux.dims(3); i++) {
    ...
    auto indices =
        matcher_.forward(predBoxes, predLogits, targetBoxes, targetClasses);
...
SetCriterion::LossDict SetCriterion::lossLabels(
  ...
  auto lossCe = weightedCategoricalCrossEntropy(
      softmaxed,
      fl::Variable(target_classes_full, false),
      weightVar,
      ReduceMode::MEAN,
      -1);
